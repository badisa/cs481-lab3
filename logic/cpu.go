...
func ParseSchedState(output string) map[string]string {
...
func ParseStat(output string) map[string]string {
    values := make(map[string]string, 2)
    statVals := strings.Split(output, " ")
    values["utime_jiffies"] = statVals[13]
    values["kernal_time_jiffies"] = statVals[14]
    return values
}

func ParseSched(output string) map[string]string {
...
func PrintSchedulerStats(procType string, format string) {
    ...
    schedStatVals := ParseSchedState(output.String())
    ...
    statVals := ParseStat(output.String())
    ...
    schedVals := ParseSched(output.String())
